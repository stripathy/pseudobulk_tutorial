...
### first plot number of sst cells per person to illustrate that there's fewer SST cells in folks with dementia than controls
sea_ad_meta %>% ggplot(aes(x = disease, y = cells_per_donor)) + geom_boxplot() + theme_cowplot() + 
  ylab('SST cells per donor')


### we'll create a cpm object which has the normalized gene expression counts for each pseudobulk sample 
pseudobulk_sst_cpm = edgeR::cpm(y = pseudobulk_sst_counts)
rownames(pseudobulk_sst_cpm) = gene_info$feature_name

pseudobulk_sst_cpm_trans = pseudobulk_sst_cpm %>% t() %>% as.data.frame()
...
pseudobulk_sst_cpm_trans %>% ggplot(aes(x = disease, y = log2(SST+1))) + geom_boxplot() + theme_cowplot() + ylab('SST expr (log2 CPM+1)')

# and show how SST normalized expression is related to numbers of cells sampled per donor
pseudobulk_sst_cpm_trans %>% ggplot(aes(x = log2(SST+1), y = log10(cells_per_donor + 1), color = disease)) + geom_point() + theme_cowplot() + 
  xlab('SST cells per donor (log10 units)') + ylab('SST expr (log2 CPM+1)')
...
# we'll also drop subjects with fewer than 50 SST cells per donor
...
sea_ad_meta[use_subjects, ] %>% group_by(disease) %>% tally
...
min_samples_expressing_gene <-  sea_ad_meta[use_subjects, ] %>% nrow * 0.8 # the 0.8 here refers to fraction of total samples that needs to express the gene
...
sex_volcano = deg_table_sex %>% ggplot(aes(x = logFC, y = -log10(P.Value), label = feature_name)) + geom_point() +
...
deg_table_disease %>% arrange(adj.P.Val) %>% head(20)


# plot volcano plot of genes associated with disease
disease_volcano = deg_table_disease %>% ggplot(aes(x = logFC, y = -log10(P.Value), label = feature_name)) + geom_point() +
